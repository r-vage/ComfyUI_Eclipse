...
- **feat:** added vertical scrollbars to the Image Selector and DOM Preview grids when there are many images to preview, preventing previews from becoming unreadably small. Implemented smart mouse-wheel detection to scroll the grid, and only bubble zoom/pan events to the canvas when the top/bottom scroll boundaries are reached or when the grid fits within the viewport.
...
- **fix:** fixed latent/image priority in KSampler (Pipe) so direct input connections on downstream samplers take priority over previous outputs in the pipe.
...
- **feat:** new combined Basic Pipe node allowing packing, unpacking, and overriding model, clip, vae, and positive/negative conditioning components dynamically.
- **feat:** updated the Eclipse KSampler (Pipe) node to take the custom `"PIPE"` output from the Smart Model Loader directly, removing the separate `basic_pipe` and `gen_data` inputs/outputs. Added required `positive` and `negative` conditioning inputs, a standard `allow_overwrite` boolean widget (which controls local settings priority), and resolved latents/images from the pipe first.

**Changed files:**

- `py/RvSampler_KSamplerPipe.py`
- `py/RvPipe_BasicPipe.py` (new)
- `__init__.py`
- `CHANGELOG.md`
- `pyproject.toml`
...
- **feat:** new Eclipse KSampler (Pipe Data) node supporting Eclipse standard inputs, with BASIC_PIPE tuple and PIPE-formatted context generation data (gen_data) outputs. The gen_data output is fully compatible with the IO Generation Data node.
- **feat:** updated the Eclipse KSampler (Pipe) node to support optional gen_data inputs and outputs (merging context generation data, where local sampler parameters overwrite incoming ones).

**Changed files:**

- `py/RvSampler_KSamplerPipeData.py` (new)
- `py/RvSampler_KSamplerPipe.py`
- `__init__.py`
- `js/eclipse-sampler-tiled-decode.js`
- `js/eclipse-seed.js`
...
- **fix:** updated the KSampler preview mechanism to render the final high-res decoded image instead of staying stuck on the low-res step preview.
- **fix:** fixed seed resolution and randomization when the seed widget is converted to an input slot and connected externally.
- **fix:** automatically hide the seed helper buttons (Randomize, New Fixed, Use Last Queued Seed) when the seed input is connected externally for KSamplers and Seed nodes.
...
- **feat:** new Eclipse KSampler and Eclipse KSampler (Pipe) custom nodes with built-in VAE / Tiled VAE decode. Denoises input latent and passes through model/vae/conditioning/pipe for clean chaining.
- **feat:** new To Basic Pipe and From Basic Pipe nodes for full compatibility with standard ComfyUI BASIC_PIPE tuples, including robust 5-tuple input error validation checks.
- **feat:** added optional `latent` and new optional `image` inputs to KSampler nodes, allowing direct image connections with automatic VAE/Tiled VAE encoding (preferring `image` over `latent` if both are connected).
- **feat:** integrated custom seed buttons (Randomize Each Time, New Fixed Random, Use Last Queued Seed) with backend seed value resolution (-1, -2, -3).
- **feat:** added `preview_mode` combo option ("Preview", "None") to show/hide the live progress render preview and final decoded image in the node body.
- **feat:** added dynamic JS widget toggling for `tile_size` which hides it when `tiled_decode` is disabled.

## 2026-06-30

### Version: 3.7.21

- **feat:** integrated GPLv3-licensed ComfyUI-Impact-Pack components under `extern/impact/` (retaining full LICENSE) to support advanced detailing workflows.
- **feat:** new Detailer (SEGS/pipe) node leveraging local `extern.impact` core logic for modular, pipeline-compatible for-each detailing cycles.
- **feat:** new Mask to SEGS node supporting both contour-based separation (`cv2.findContours`) and combined bboxes, complete with padding and boundary coordinate corrections.
- **feat:** CLIP Text Encode (Advanced) — added `krea2_only_multiplier` option to selectively bypass scaling on non-Krea2 models and improved input tooltips.
- **feat:** Load Batch From Folder — added real-time node title progress tracking (Scanning/Probing/Loading/Resizing) with try-finally title restoration to both standard and advanced nodes.
- **fix:** SEGS Preview nodes — corrected coordinate scaling ratios (x-axis uses width ratio, y-axis uses height ratio) and fixed out-of-bounds crop/mask indices during batch runs.
- **fix:** Florence-2 model modeling — backported standard 4D attention mask builders for SDPA and causal/non-causal attention configurations.
- **fix:** Image Comparer — set `is_input_list=True` and implemented robust batch/list tensor unpacking to prevent duplicate temp images when mixing batches and lists, with proper fallback handling for unconnected outputs.
- **fix:** Smart LM Loader — fixed a critical bug in Ollama vision generation where multi-turn message history (including task-specific few-shot examples) was completely discarded, and normalized list-based few-shot content to clean strings for Ollama compatibility.
- **fix:** Smart LM Loader — unified prompt merging for LLaVA and Mllama models under Hugging Face Transformers backend to ensure system instructions are properly sent when the user prompt is empty.
- **fix:** Smart LM Loader — added empty-message text guards to vLLM, SGLang, and llama.cpp vision backends to prevent empty user message errors.
- **fix:** Smart LM Loader — fixed model downloader name error `name 'inspect' is not defined` when resolving corrupted files.
- **fix:** Smart LM Loader — added remote repo scanning and local weight-check heuristics to the model completeness check to detect and automatically recover from aborted or partially downloaded models.
- **fix:** Smart LM Loader — updated path resolution to check directory completeness rather than just checking directory existence, preventing local loading attempts on incomplete model folders.
- **fix:** resolved AttributeError: 'NoneType' object has no attribute 'extend' inside the detailer loop by correcting MetadataWrapper in the Mask to SEGS node.
- **refactor:** renamed CLIPTextEncode files and classes from `RvText_CLIPTextEncode*` to `RvCond_CLIPTextEncode*` to align with their conditioning output type.
- **refactor:** cleaned up unused node classes and helpers under `extern/impact/` (core.py and impact_pack.py) to minimize code bloat, and customized log prefixes to '[Eclipse Impact]' for console clarity.
- **refactor:** consolidated frontend layout by renaming `js_src/` to `js/` to serve unminified JS source files directly, removing `build_js.py` minification and nested git metadata.
- **refactor:** code maintenance/quality

**Changed files:**

- `py/RvDetailer_ForEachPipe.py` (new)
- `py/RvMask_ToSEGS.py` (new)
...
- `js/eclipse-node-title-updater.js` (new)
...
- `py/RvText_CLIPTextEncode.py` (delete)
- `py/RvText_CLIPTextEncodeAdvanced.py` (delete)
- `py/RvImage_SEGSPreview.py`
- `py/RvImage_SEGSPreview_Simple.py`
- `extern/florence2/modeling_florence2.py`
- `extern/impact/` (new & optimized)
- `js/` (unminified source layout)
- `__init__.py`
- `core/sml/model_files.py`
...
- `py/RvLoader_SmartModelLoader_LM.py`
- `py/RvSettings_SmartSamplerSettings.py`
- `py/RvSettings_SmartSamplerSettings_v2.py`
- `py/RvText_ReplaceString_Adv.py`
...
- `py/RvTools_VRAMCleanUp.py`
- `py/RvVideo_LoopCalc.py`
...
- `py/RvLogic_Integer.py`
- `py/RvLogic_Integer_Gen.py`
- `py/RvLogic_Seed.py`
- `py/RvLogic_Seed_32bit.py`
...
- **fix:** Krea2 resolution presets — updated the resolution selector options to match the 8 official aspect ratios (1:1, 4:3, 3:2, 16:9, 2.35:1, 4:5, 2:3, 9:16) supported on Krea.ai at 1K base resolution (long edge is 1024 pixels) instead of the previous incorrect 768px base values.
...
- **fix:** Model Loader — refined tooltips to remove confusing SDXL references for standalone UNets and external CLIP files, since SDXL is typically loaded as a baked standard checkpoint.
- **fix:** Image Comparer — fixed batch/list comparison where the slider always compared with the first frame (B1). All images of the batch/list are now saved and passed to the UI, active selections are preserved across execution updates, and Side A/B selected frame indices are automatically synchronized.
...
- js/eclipse-sml-loader.js, js/eclipse-sml-detection.js, js/eclipse-ui-enhancements.js (new/edited)
...
- py/RvImage_Resize.py, js/eclipse-image-resize.js (new)
- py/RvFolder_SmartFolder.py
- py/RvLoader_SmartModelLoader.py
...
- js/eclipse-smart-loader-v2.js
- js/eclipse-smart-loader-plus-v2.js
- js/eclipse-smart-loader-basic-v2.js
- js/eclipse-smart-folder.js
- js/eclipse-smart-folder-v2.js
- js/eclipse-smart-prompt.js
- js/eclipse-smart-prompt-v2.js
- js/eclipse-smart-sampler-settings.js
- js/eclipse-smart-sampler-settings-v2.js
- js/eclipse-replace-string-v3.js
- js/eclipse-save-images-v2.js
- js/eclipse-prompt-styler.js
- js/eclipse-clip-loader.js
- js/eclipse-sampler-overwrite.js
...
- py/RvText_SmartPromptV2.py, js/eclipse-smart-prompt-v2.js (new)
...
- js/eclipse-combo-chip.js (factory refactor: remove backingWidget, hooks, add string serialization)
- js/eclipse-smart-model-loader.js (single-path, string serializeValue)
- js/eclipse-model-loader.js (single-path)
- js/eclipse-smart-sampler-settings.js (single-path)
- js/eclipse-smart-sampler-settings-v2.js (single-path, string serializeValue)
- js/eclipse-replace-string-v3.js (single-path)
- js/eclipse-save-images-v2.js (single-path)
- js/eclipse-smart-prompt-v2.js (single-path)
- js/eclipse-widget-performance-utils.js (removeSocketlessInputs + tooltip fix)
- js/eclipse-ui-enhancements.js (global socketlessFix hook)
...
- js/eclipse-load-image.js, js/eclipse-prompt-styler.js
- js/eclipse-dynamic-inputs.js (remove converted-widget type override for inputcount)
- js/eclipse-dom-preview.js (add getMinHeight/getMaxHeight to widget options)
...
- js/eclipse-save-images-v2.js (new)
- js/eclipse-set-get.js, js/eclipse-getfirst.js, js/eclipse-getallactive.js
- js/eclipse-replace-string-v3.js, js/eclipse-smart-folder-v2.js
- js/eclipse-smart-folder.js, js/eclipse-clip-loader.js
- js/eclipse-sampler-overwrite.js, js/eclipse-model-loader.js
- js/eclipse-smart-model-loader.js
- js/eclipse-image-resolution.js, js/eclipse-video-resolution.js
- js/eclipse-save-prompt.js
...
- js/eclipse-set-get.js (new)
- js/eclipse-dom-preview.js (new)
- js/eclipse-dom-preview-nodes.js (new)
- js/eclipse-dom-text.js (new)
- js/eclipse-string-nodes.js (new)
- js/eclipse-preview-culling.js (new)
- py/RvImage_Preview_Image_DOM.py (new)
- js/eclipse-show-any.js
- js/eclipse-load-image.js
- js/eclipse-load-image-folder.js (merged)
- js/eclipse-load-image-folder-pipe.js (deleted)
- js/eclipse-getfirst.js
- js/eclipse-getallactive.js
- js/eclipse-ui-enhancements.js
