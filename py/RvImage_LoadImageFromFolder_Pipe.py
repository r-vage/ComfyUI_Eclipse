...
def _load_image_with_metadata(
    filepath: str, extract_metadata: bool = False
...
class RvImage_LoadImageFromFolder_Pipe(io.ComfyNode):
        ...
        return io.Schema(
            ...
            inputs=[
                io.String.Input(
                    ...
                    default="",
                    multiline=True,
                    tooltip="Path(s) to folder(s) containing images. One folder per line. Can be absolute or relative to ComfyUI input folder.",
                ),
                io.Boolean.Input(
                    "include_subfolders",
                    default=True,
                ...
                io.Boolean.Input(
                    ...
                    default=False,
                    socketless=True,
                    tooltip="Extract generation metadata from images (slower). Disable for faster loading.",
                ),
                io.Boolean.Input(
                    "refresh_list",
                    default=False,
                    socketless=True,
                    tooltip="Force refresh of the cached file list.",
                ),
                io.Int.Input(
                    "seed_input",
                    force_input=True,
                    optional=True,
                    tooltip="When connected, special index modes only advance when this value changes.",
                ),
            ],
            outputs=[
    ...
    def execute(
        ...
        if stop_at_end and index == total_count:
            log.msg(
                _LOG_PREFIX,
                f"Reached end ({total_count} images in {total_folders} folders). Stopping workflow.",
            )
        ...
        while attempts < total_count:
            ...
            current_image, current_mask, pipe = _load_image_with_metadata(
                current_filepath, extract_metadata
            )
            ...
            if stop_at_end and current_index < start_index:
                log.msg(
                    _LOG_PREFIX,
                    f"Reached end after skipping failed images. Stopping workflow.",
                )
                PromptServer.instance.send_sync("stop-iteration", {})
                nodes.interrupt_processing()
                empty_pipe = {"stopped": True, "reason": "end_of_folders"}
