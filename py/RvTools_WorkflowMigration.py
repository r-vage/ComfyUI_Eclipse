...
class RvTools_WorkflowMigration(io.ComfyNode):
    ...
    def define_schema(cls):
        return io.Schema(
            ...
            inputs=[
                io.String.Input("path", default="", tooltip="Absolute path to a single .json workflow file or a folder containing workflows"),
                io.String.Input("mapping_file", default="tools/migration_eclipse.txt, tools/migration_rvtoolsv1.txt, tools/migration_rvtoolsv2.txt", tooltip="Filename or path (absolute or relative to repo root) to a custom migration mappings text file (comma-separated for multiple files)"),
                io.Boolean.Input("run_migration", default=False, label_on="write changes", label_off="dry run", socketless=True,
                    tooltip="If disabled, only simulates changes (dry run). If enabled, writes updates and creates backups."),
                io.Boolean.Input("create_backup", default=True, label_on="yes", label_off="no", socketless=True,
    ...
    @classmethod
    def execute(cls, path: str, mapping_file: str, run_migration: bool, create_backup: bool):
        path = path.strip()
        if not path:
            return io.NodeOutput("Error: Path is empty. Please enter a valid workflow file or folder path.")
        ...
        for mapping_path in paths_to_load:
            ...
            if os.path.exists(resolved_mapping_path) and os.path.isfile(resolved_mapping_path):
                try:
                    ...
                    log.error(_LOG_PREFIX, f"Failed to read mapping file '{resolved_mapping_path}': {e}")
        ...
        mode_str = "WRITE MODE" if run_migration else "DRY-RUN MODE (No changes written)"
        map_source = "None (Using Fallback Mappings)" if using_fallback else ", ".join(loaded_sources)
        log_lines = [
            "============================================================",
        ...
        for file_path in files_to_process:
            ...
            sorted_mappings = sorted(active_mappings.items(), key=lambda x: len(x[0]), reverse=True)
            ...
            if not replacements:
                log_lines.append(f"• [Skip] {os.path.basename(file_path)} — No legacy Eclipse/RvTools node IDs found.")
                pbar.update(1)
                continue
            ...
            if run_migration:
                ...
                if create_backup:
                    ...
                    try:
                        ...
                        log_lines.append(f"    - Backup saved: {os.path.basename(backup_path)}")
                    except Exception as e:
        ...
        log_lines.append("------------------------------------------------------------")
        if run_migration:
            log_lines.append(f"Migration finished. Successfully updated {total_changed} file(s).")
        else:
            log_lines.append(f"Dry-run finished. Found {total_changed} file(s) that require migration.")
        log_lines.append("============================================================")

        full_logs = "\n".join(log_lines)
        log.msg(_LOG_PREFIX, f"Execution completed. {total_changed} files updated/found.")
