...
class FilenameProcessor:
    ...
    def sanitize_path(path: str) -> str:
        ...
        if len(sanitized_path) > 255:
            log.warning(
                _LOG_PREFIX,
                f"Path too long, may cause issues on some systems: {sanitized_path}",
            )
...
def _resolve_filename_prefix(output_path: str, filename_prefix: str) -> tuple[str, str]:
    # Allow a relative directory path in filename_prefix while keeping saves inside output_path.
    processed_prefix = filename_processor.sanitize_path(
        string_placeholder(filename_prefix, True)
    )
    relative_directory, filename = os.path.split(processed_prefix)
    save_directory = os.path.abspath(os.path.join(output_path, relative_directory))
    if os.path.commonpath((output_path, save_directory)) != output_path:
        raise ValueError("filename_prefix must resolve inside the configured output path.")
    os.makedirs(save_directory, exist_ok=True)
    return save_directory, filename_processor.sanitize_filename(filename)
        ...
        for key, value in values.items():
            ...
            if not isinstance(value, expected_type):  # type: ignore[arg-type]
                try:
                    ...
                    log.debug(
                        _LOG_PREFIX,
                        f"Ignoring non-numeric/invalid value for {key}: {e}",
                    )
                    value = ""
    ...
    if not matching:
        if os.sep in name or "/" in name:
            candidate = os.path.join(
                folder_paths.get_folder_paths("embeddings")[0], name
            )
    ...
    if not matching:
        for ext in [".pt", ".safetensors", ".bin"]:
            ...
            candidate_path = os.path.join(
                folder_paths.get_folder_paths("embeddings")[0], candidate
            )
...
def full_lora_path_for(lora: str):
    ...
    if not matching:
        for ext in [".safetensors", ".pt", ".bin"]:
            candidate = name if name.lower().endswith(ext) else name + ext
            candidate_path = os.path.join(
                folder_paths.get_folder_paths("loras")[0], candidate
            )
            if os.path.exists(candidate_path):
                matching = candidate
                break
    ...
    def __perform(self, prompts):
        for prompt in prompts:
            for embedding in re.findall(
                self.EMBEDDING, prompt, re.IGNORECASE | re.MULTILINE
            ):
...
class RvImage_Save(io.ComfyNode):
        ...
        return io.Schema(
            ...
            inputs=[
                ...
                io.Boolean.Input(
                    ...
                    label_off="no",
                    socketless=True,
                    tooltip="Use lossless compression for WebP",
                ),
                io.Boolean.Input(
                    "embed_workflow",
                    default=True,
                    ...
                    label_off="no",
                    socketless=True,
                    tooltip="Embed workflow in image metadata",
                ),
                io.Boolean.Input(
                    ...
                    label_off="no",
                    socketless=True,
                    tooltip="Save A1111-compatible generation data",
                ),
                io.Boolean.Input(
                    "remove_prompts",
                    default=False,
                    ...
                    label_off="no",
                    socketless=True,
                    tooltip="Remove prompts from metadata",
                ),
                io.Boolean.Input(
                ...
                io.Boolean.Input(
                    ...
                    label_off="no",
                    socketless=True,
                    tooltip="Add LoRA tags to prompt metadata",
                ),
                io.Boolean.Input(
                    "show_previews",
                    default=True,
                    ...
                    label_off="no",
                    socketless=True,
                    tooltip="Show image previews in UI",
                ),
                io.Boolean.Input(
                    "save_to_disk",
                    default=True,
                    ...
                    label_off="no",
                    socketless=True,
                    tooltip="Save images to disk (disable for preview-only mode)",
                ),
                ...
                io.Boolean.Input(
                    ...
                    label_off="no",
                    socketless=True,
                    tooltip="Enable custom quality (default: 100)",
                ),
                io.Boolean.Input(
                    "use_dpi",
                    default=False,
                    ...
                    label_off="no",
                    socketless=True,
                    tooltip="Enable custom DPI (default: 300)",
                ),
                io.Boolean.Input(
                    "use_output",
                    default=True,
                    ...
                    label_off="no",
                    socketless=True,
                    tooltip="Enable custom output path (default: ComfyUI output folder)",
                ),
                io.Boolean.Input(
                    "use_filename",
                    default=True,
                    ...
                    label_off="no",
                    socketless=True,
                    tooltip="Enable custom filename (default: ComfyUI prefix)",
                ),
                # Output path (shown when output chip active)
                io.String.Input(
                    "output_path",
                    default=r"%Y-%M-%D\%basemodel",
                    tooltip="Output path. Placeholders: %today, %date, %time, %Y, %m/%M, %d/%D, %H, %S, %basemodel, %model, %seed, %sampler_name, %scheduler, %steps, %cfg, %denoise, %clip_skip",
                ),
                # Filename widgets (shown when filename chip active)
                io.String.Input(
                    "filename_prefix",
                    default="%basemodel, %seed, %sampler_name, %scheduler, %steps_steps",
                    tooltip="Filename prefix. Placeholders: %today, %date, %time, %Y, %m/%M, %d/%D, %H, %S, %basemodel, %model, %seed, %sampler_name, %scheduler, %steps, %cfg, %denoise, %clip_skip",
                ),
                io.String.Input(
                    "filename_delimiter", default="_", tooltip="Filename delimiter"
                ),
                io.Int.Input(
                    ...
                    max=9,
                    step=1,
                    tooltip="Number of digits for counter padding",
                ),
                io.Boolean.Input(
                    ...
                    label_on="yes",
                    label_off="no",
                    tooltip="Number at start of filename",
                ),
                # Always-visible
                io.Combo.Input(
                    "extension",
                    options=["png", "jpg", "jpeg", "gif", "tiff", "webp", "bmp"],
                ...
                io.Int.Input(
                    "dpi", default=300, min=1, max=2400, step=1, tooltip="Image DPI"
                ),
                # Optional inputs
            ...
            outputs=[
                ...
                io.String.Output("files", is_output_list=True),
            ],
    ...
    def execute(
        ...
        optimize_image = unwrap_value(optimize_image)
        lossless_webp = unwrap_value(lossless_webp)
        embed_workflow = unwrap_value(embed_workflow)
        save_generation_data = unwrap_value(save_generation_data)
        remove_prompts = unwrap_value(remove_prompts)
        save_workflow_as_json = unwrap_value(save_workflow_as_json)
        add_loras_to_prompt = unwrap_value(add_loras_to_prompt)
        show_previews = unwrap_value(show_previews)
        save_to_disk = unwrap_value(save_to_disk)
        use_quality = unwrap_value(use_quality)
        use_dpi = unwrap_value(use_dpi)
        use_output = unwrap_value(use_output)
        use_filename = unwrap_value(use_filename)
        output_path = unwrap_value(output_path)
        filename_prefix = unwrap_value(filename_prefix)
        filename_delimiter = unwrap_value(filename_delimiter)
        filename_number_padding = unwrap_value(filename_number_padding)
        filename_number_start = unwrap_value(filename_number_start)
        extension = unwrap_value(extension)
        quality = unwrap_value(quality)
        dpi = unwrap_value(dpi)
        ...
        if images is None and pipe_opt is None:
            raise RuntimeError(
                "Save Images v2 requires either an image input or a pipe input (pipe_opt)."
            )
        ...
        if pipe_opt is not None:
            if isinstance(pipe_opt, list) and len(pipe_opt) > 0:
                pipe_opt = pipe_opt[0]
            if isinstance(pipe_opt, tuple) and len(pipe_opt) > 0:
                ...
                raise ValueError(
                    "Save Images v2 expects dict-style or tuple-style pipes for pipe_opt."
                )
        ...
        if images is None and pipe_opt is not None:
            ...
            if pipe_images is None:
                raise RuntimeError(
                    "Save Images v2: pipe_opt provided but contains no 'images' data."
                )

            if isinstance(pipe_images, torch.Tensor):
                if pipe_images.numel() == 0:
                    raise RuntimeError(
                        "Save Images v2: pipe_opt contains an empty tensor for 'images'."
                    )
                images = (
                    [pipe_images[i] for i in range(pipe_images.size(0))]
                ...
                if pipe_images.size == 0:
                    raise RuntimeError(
                        "Save Images v2: pipe_opt contains an empty numpy array for 'images'."
                    )
                images = (
                    [pipe_images[i] for i in range(pipe_images.shape[0])]
        ...
        if images is not None:
            if (
                ...
                was_list = False
                original_images_tensor = images[0]
            elif isinstance(images, (torch.Tensor, np.ndarray)) and images.ndim == 4:
                was_list = False
                original_images_tensor = images

        if (
            isinstance(original_images_tensor, (torch.Tensor, np.ndarray))
            and original_images_tensor.ndim == 4
        ):
            was_list = False

        # Normalize: flatten any nested lists/tuples and 4D tensors in list (from ConcatMulti shape-mismatch)
        images = [img.squeeze(0) for img in flatten_images(images)]
        ...
        if not save_to_disk:
            ...
            for batch_number, image in enumerate(images):
                ...
                full_output_folder, filename_base, counter_temp, subfolder, _ = (
                ...
                counter_temp += 1
            if not was_list:
                out_img = [original_images_tensor]
            else:
                out_img = original_images_tensor
            return io.NodeOutput(out_img, [], ui={"images": results})
        ...
        if pipe_opt is not None and isinstance(ctx, dict):
            ...
            denoise = (
                ctx.get("denoise")
                if ctx.get("denoise") is not None
                else ctx.get("guidance")
            )
            ...
            positive = (
                _prompt_to_str(positive)
                if positive not in (None, "", "undefined", "none")
                else ""
            )
            negative = (
                _prompt_to_str(negative)
                if negative not in (None, "", "undefined", "none")
                else ""
            )
            ...
            if models:
                ...
                global_values["basemodel"] = return_filename_without_extension(
                    first_model
                )
                    ...
                    for search_dir in search_dirs:
                        for ext in extensions:
                            pattern = os.path.join(
                                search_dir,
                                "**",
                                model if model.lower().endswith(ext) else model + ext,
                            )
                ...
                for model in models:
                    model_path, model_dir = find_model_file(
                        model, search_dirs, extensions
                    )
                    if model_path and os.path.exists(model_path):
                        ...
                        if modelhash:
                            ...
                            if model_dir and model_dir in upscale_model_dirs:
                                ...
                                model_key = civitai_model_key_name(
                                    return_filename_without_extension(model)
                                )
                            model_string[model_key] = modelhash
                    else:
                        log.warning(
                            _LOG_PREFIX,
                            f"Model file not found for hash: {model} (path: {model_path}, dir: {model_dir})",
                        )
            ...
            if not lora_names in (None, "", "undefined", "none"):
                ...
                metadata_extractor = PromptMetadataExtractor(
                    [positive_with_loras, negative]
                )
            ...
            if extra_pnginfo is not None and collected_airs:
                try:
                    ...
                    if isinstance(workflow_data, dict):
                        if "extra" not in workflow_data or not isinstance(
                            workflow_data["extra"], dict
                        ):
                            workflow_data["extra"] = {}
                        if "airs" not in workflow_data["extra"] or not isinstance(
                            workflow_data["extra"]["airs"], list
                        ):
                        ...
                        if added_any:
                            ...
                            log.msg(
                                _LOG_PREFIX,
                                f"Injected {len(collected_airs)} AIRs into workflow metadata: {collected_airs}",
                            )
                except Exception as e:
                    log.error(
                        _LOG_PREFIX,
                        f"Failed to inject AIRs into workflow metadata: {e}",
                    )
            ...
            if sampler_name and sampler_name not in (None, "", "undefined", "none"):
                civitai_sampler_name = _get_civitai_sampler_name(
                    sampler_name.replace("_gpu", ""), scheduler
                )
            else:
            ...
            clip_skip_segment = (
                f", Clip skip: {clip_skip_value_for_meta}"
                if clip_skip_value_for_meta is not None
                else ""
            )
        ...
        save_directory, filename_prefix = _resolve_filename_prefix(output_path, filename_prefix)
        ...
        for image in images:
            ...
            if extension == "webp":
                ...
                try:
                    log.debug(
                        _LOG_PREFIX, f"WEBP parameters (diagnostic): {a111_params}"
                    )
                except Exception:
                    pass
                ...
                if pipe_opt != None and save_generation_data:
                    ...
                    if lora_weights:
                        try:
                            ...
                            log.error(
                                _LOG_PREFIX, f"Failed to add lora_weights metadata: {e}"
                            )
            ...
            try:
                ...
                if extension in ["jpg", "jpeg"]:
                    ...
                    img.save(save_path, pnginfo=exif_data, optimize=optimize_image)
                elif extension == "bmp":
                    img.save(save_path)
                elif extension == "tiff":
                    img.save(save_path, quality=quality, optimize=optimize_image)
                else:
                    img.save(save_path, pnginfo=exif_data, optimize=optimize_image)
                ...
                log.error(
                    _LOG_PREFIX, "Unable to save file due to the following error:"
                )
                log.error(_LOG_PREFIX, str(e))

            if save_workflow_as_json:
                output_json = os.path.abspath(os.path.join(save_directory, jsonfile))
        ...
        if show_previews == True:
            return io.NodeOutput(
                out_img, out_files, ui={"images": results, "files": output_files}
            )
        else:
            return io.NodeOutput(out_img, out_files, ui={"images": []})
