...
class RvImage_Save(io.ComfyNode):
    ...
    def define_schema(cls):
        return io.Schema(
            ...
            inputs=[
                ...
                io.String.Input("filename_prefix", default="%basemodel, %seed, %sampler_name, %scheduler, %steps_steps", tooltip="Filename prefix. Placeholders: %today, %date, %time, %Y, %m/%M, %d/%D, %H, %S, %basemodel, %model, %seed, %sampler_name, %scheduler, %steps, %cfg, %denoise, %clip_skip"),
    ...
    def execute(cls,
        ...
        optimize_image = _unwrap(optimize_image)
        lossless_webp = _unwrap(lossless_webp)
        embed_workflow = _unwrap(embed_workflow)
        save_generation_data = _unwrap(save_generation_data)
        remove_prompts = _unwrap(remove_prompts)
        save_workflow_as_json = _unwrap(save_workflow_as_json)
        add_loras_to_prompt = _unwrap(add_loras_to_prompt)
        show_previews = _unwrap(show_previews)
        save_to_disk = _unwrap(save_to_disk)
        use_quality = _unwrap(use_quality)
        use_dpi = _unwrap(use_dpi)
        use_output = _unwrap(use_output)
        use_filename = _unwrap(use_filename)
        output_path = _unwrap(output_path)
        filename_prefix = _unwrap(filename_prefix)
        filename_delimiter = _unwrap(filename_delimiter)
        filename_number_padding = _unwrap(filename_number_padding)
        filename_number_start = _unwrap(filename_number_start)
        extension = _unwrap(extension)
        quality = _unwrap(quality)
        dpi = _unwrap(dpi)
        ...
        if images is not None:
            if isinstance(images, list) and len(images) == 1 and isinstance(images[0], torch.Tensor) and images[0].dim() == 4:
                was_list = False
        ...
        if isinstance(original_images_tensor, (torch.Tensor, np.ndarray)) and original_images_tensor.ndim == 4:
            was_list = False
        ...
        _flat = []
        flat_images = _flatten_list(images)
        for _img in flat_images:
        ...
        if show_previews == True:
            return io.NodeOutput(out_img, out_files, ui={"images": results, "files": output_files})
        else:
