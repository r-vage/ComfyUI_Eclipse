...
class RvImage_TileSplit(io.ComfyNode):
        ...
        return io.Schema(
            ...
            inputs=[
                ...
                io.Vae.Input(
                    "vae",
                    optional=True,
                    tooltip="VAE for spatial ratio detection. "
            ...
            outputs=[
                io.Image.Output(
                    "images",
                    tooltip="List of tile images (one per tile).",
                    is_output_list=True,
                ),
                io.Custom(_TILE_PIPE_TYPE).Output(
                    "tile_pipe",
    ...
    def execute(
        ...
        width_factor = unwrap_value(width_factor, 2)
        height_factor = unwrap_value(height_factor, 3)
        overlap_rate = unwrap_value(overlap_rate, 0.15)
        vae = unwrap_value(vae, None)
        align_override = unwrap_value(align_override, 0)

        # Flatten list of images
        flat_images = [img.squeeze(0) for img in flatten_images(image)]

        if not flat_images:
            return io.NodeOutput([], {})

        # Use first image for splitting
        target_tensor = flat_images[0].unsqueeze(0)
        ...
        _, raw_H, raw_W, _ = target_tensor.shape
        ...
        if overlap_rate == 0:
            tile_w = (
                raw_W
                if width_factor == 1
                else _align_up(raw_W // width_factor, align_to)
            )
            tile_h = (
                raw_H
                if height_factor == 1
                else _align_up(raw_H // height_factor, align_to)
            )
        else:
            tile_w = (
                raw_W
            ...
            tile_h = (
                ...
                if height_factor == 1
                    ...
                    int(raw_H / (1 + (height_factor - 1) * (1 - overlap_rate))),
                    align_to,
                )
            )
        ...
        log.msg(
            _LOG_PREFIX,
            f"{raw_W}x{raw_H} → tiles {tile_w}x{tile_h} "
            f"(grid={width_factor}x{height_factor}, overlap={overlap_rate}, "
            f"align={align_to} [{source}])",
        )

        # Split all images into tiles
        tiles_list = []
        tile_pipes = []

        for img_tensor in flat_images:
            pil_img = _tensor2pil(img_tensor)
            ...
            if img_w <= tile_w and img_h <= tile_h:
                ...
                tiles_list.append(img_tensor.unsqueeze(0))
                tile_pipes.append(tile_pipe)
                continue
            ...
            for row in range(num_rows):
                for col in range(num_cols):
                    ...
                    tiles_list.append(_pil2tensor(tile).unsqueeze(0))
        ...
        ret_pipe = tile_pipes if len(tile_pipes) > 1 else tile_pipes[0]

        return io.NodeOutput(tiles_list, ret_pipe)
