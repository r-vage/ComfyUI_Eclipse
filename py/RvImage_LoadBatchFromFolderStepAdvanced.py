...
class RvImage_LoadBatchFromFolderStepAdvanced(io.ComfyNode):
    @classmethod
    def define_schema(cls):
        return io.Schema(
            node_id="Load Batch From Folder (Step Advanced) [Eclipse]",
            display_name="Load Files From Folder (Step)",
            ...
            inputs=[
                ...
                io.Int.Input(
                    ...
                    tooltip="Batch index. When increased, shifts the frame start/end to load the next batch of the same size.",
                ),
                io.Combo.Input(
                    "resize_mode",
            ...
            outputs=[
                ...
                io.Mask.Output(
                    "masks",
                    is_output_list=True,
                    tooltip="Corresponding masks batch or list.",
                ),
                io.String.Output(
                    "files",
                    is_output_list=True,
                    tooltip="List of absolute file paths matching the loaded frames/images.",
                ),
            ],
    ...
    def execute(
        ...
        if cache_key in _loaded_tensors_cache:
            ...
            cached_images, cached_masks, cached_files = _loaded_tensors_cache[cache_key]
            return io.NodeOutput(cached_images, cached_masks, cached_files)
        ...
        try:
            ...
            if all_known:
                ...
                s_base, e_base = _resolve_slice(total_probe, frame_start, frame_end)
                batch_size = e_base - s_base + 1
                s = s_base + batch_index * batch_size
                e = e_base + batch_index * batch_size
                frames_to_load = max(0, min(e, total_probe - 1) - s + 1)
                log.msg(
                    _LOG_PREFIX,
                ...
                for idx, (fp, file_count) in enumerate(zip(all_files, file_counts)):
                    ...
                    if fp.lower().endswith(_VIDEO_EXTENSIONS):
                        ...
                        if not vid_frames:
                            ...
                            filepaths_list.extend([fp] * len(vid_frames))
            ...
            if not range_applied:
                ...
                s_base, e_base = _resolve_slice(total_raw, frame_start, frame_end)
                batch_size = e_base - s_base + 1
                s = s_base + batch_index * batch_size
                e = e_base + batch_index * batch_size
                log.msg(
                    _LOG_PREFIX,
                    f"Frame range [{frame_start}, {frame_end}] with batch_index {batch_index} → keeping frames {s}–{e} of {total_raw}",
                )
                images = images[s : e + 1]
                masks = masks[s : e + 1]
            ...
            if resize_mode == "custom":
                ...
                return io.NodeOutput(resized_images, resized_masks, filepaths_list)

            # List mode: skip resizing and stacking — return as a Python list.
            if resize_mode == "list":
                return io.NodeOutput(images, masks, filepaths_list)
            ...
            _loaded_tensors_cache[cache_key] = (images, masks, filepaths_list)
            return io.NodeOutput(images, masks, filepaths_list)
        finally:
            restore_title()
