...
def _save_images_to_temp(image_tensor, side, metadata=None):
...
class RvImage_ImageComparer(io.ComfyNode):
    ...
    def execute(cls, image_a=None, image_b=None):
        ...
        # Convert back to concatenated 4D tensors for saving and output
        tensor_a = torch.cat(list_a, dim=0) if list_a else None
        tensor_b = torch.cat(list_b, dim=0) if list_b else None

        if tensor_a is not None and tensor_a.shape[0] > 0:
            ui_data["a_images"] = _save_images_to_temp(tensor_a, "A", metadata)

        if tensor_b is not None and tensor_b.shape[0] > 0:
            ui_data["b_images"] = _save_images_to_temp(tensor_b, "B", metadata)
        ...
        a_ok = tensor_a is not None and tensor_a.shape[0] > 0
        b_ok = tensor_b is not None and tensor_b.shape[0] > 0
        if default_side == "a":
            output_image = tensor_a if a_ok else tensor_b
        else:
            output_image = tensor_b if b_ok else tensor_a
