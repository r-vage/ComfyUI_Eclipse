...
class RvImage_LoadImagePath(io.ComfyNode):
	@classmethod
	def define_schema(cls):
		return io.Schema(
			...
			outputs=[
				...
				io.Int.Output("steps"),
				io.Combo.Output("sampler"),
				io.Combo.Output("scheduler"),
				io.Float.Output("cfg_scale"),
				io.Int.Output("seed"),
				io.String.Output("version"),
			],
		)
	...
	def execute(cls, image):
		...
		return io.NodeOutput(image, mask, pipe["text_pos"], pipe["text_neg"], pipe["width"], pipe["height"],
			pipe["steps"], pipe["sampler_name"], pipe["scheduler"], pipe["cfg"], pipe["seed"], "")
