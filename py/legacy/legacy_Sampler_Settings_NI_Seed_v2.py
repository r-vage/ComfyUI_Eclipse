...
class RvSettings_Sampler_Settings_NI_Seed_v2(io.ComfyNode):
    @classmethod
    def define_schema(cls):
        return io.Schema(
            node_id="Sampler Settings NI+Seed v2 [Eclipse]",
            display_name="⚠ Sampler Settings NI+Seed v2",
            ...
            inputs=[
                ...
                io.Float.Input("denoise_upscale", default=0.45, min=0, max=1.0, step=0.01, tooltip="Denoise strength for upscale pass (0-1)."),
    ...
    def execute(cls, allow_overwrite: bool, sampler_name: str, scheduler: str, steps: int, cfg: float,
                guidance: float, denoise: float, denoise_upscale: float, upscale_value: float,
                sigmas_denoise: float, noise_strength: float, seed: int = 0) -> io.NodeOutput:
        ...
        pipe = {
            ...
            "denoise_upscale": float(denoise_upscale),
