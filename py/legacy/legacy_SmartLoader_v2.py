...
if not _support_messages_printed:
    ...
    if nunchaku_info['available']:
        log.debug(_LOG_PREFIX, f"✓ Nunchaku support: {nunchaku_info['version']}")
    
    if GGUF_AVAILABLE:
        log.debug(_LOG_PREFIX, "✓ GGUF support available")

class RvLoader_SmartLoader_v2(io.ComfyNode):
    ...
    def define_schema(cls):
        ...
        return io.Schema(
            node_id="Smart Loader v2 [Eclipse]",
            display_name="⚠ Smart Loader v2",
    ...
    def execute(cls, **kwargs):
        ...
        if is_standard:
            ...
            if ext not in safe_exts:
                log.warning(_LOG_PREFIX, f"Warning: '{zimage_name}' uses extension '{ext}'. Consider .safetensors.")
