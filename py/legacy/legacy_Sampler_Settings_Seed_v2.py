...
class RvSettings_Sampler_Settings_Seed_v2(io.ComfyNode):
    @classmethod
    def define_schema(cls):
        return io.Schema(
            node_id="Sampler Settings+Seed v2 [Eclipse]",
            display_name="⚠ Sampler Settings+Seed v2",
    ...
    def execute(cls, allow_overwrite: bool, sampler_name: Any, scheduler: Any, steps: int, cfg: float,
                guidance: float, denoise: float, upscale_steps: int, upscale_denoise: float,
                upscale_value: float, seed: int = 0) -> io.NodeOutput:
