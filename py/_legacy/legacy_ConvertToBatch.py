    ...
    try:
        ...
        if isinstance(images, torch.Tensor) and images.ndim == 4:
            if images.shape[0] > 0:
                log.msg(
                    _LOG_PREFIX,
                    f"image_list_to_batch: Already a batch tensor with shape {images.shape}",
                )
                return (images,)
            else:
                log.warning(
                    _LOG_PREFIX, "image_list_to_batch: Empty batch, returning as-is"
                )
                return (images,)
        ...
        if not isinstance(images, (list, tuple)):
            log.warning(
                _LOG_PREFIX, "image_list_to_batch: Not a list or tuple, returning as-is"
            )
            return (images,)
        ...
        log.msg(
            _LOG_PREFIX, f"image_list_to_batch: Processing list of {len(images)} images"
        )
        ...
        for i, image2 in enumerate(images[1:], 1):
            ...
            if image1.shape[1:] != image2.shape[1:]:
                log.msg(
                    _LOG_PREFIX,
                    f"image_list_to_batch: Upscaling image {i} from {image2.shape[1:3]} to {image1.shape[1:3]}",
                )
        ...
        log.msg(
            _LOG_PREFIX,
            f"image_list_to_batch: Created batch tensor with shape {image1.shape}",
        )
        return (image1,)
    except Exception as e:
        log.error(
            _LOG_PREFIX, f"image_list_to_batch conversion failed: {e}, returning as-is"
        )
        import traceback
    ...
    try:
        ...
        if isinstance(mask, torch.Tensor) and mask.ndim in (3, 4):
            if mask.shape[0] > 0:
                log.msg(
                    _LOG_PREFIX,
                    f"mask_list_to_batch: Already a batch tensor with shape {mask.shape}",
                )
                return (mask,)
            else:
                log.warning(
                    _LOG_PREFIX, "mask_list_to_batch: Empty batch, returning as-is"
                )
                return (mask,)
        ...
        if not isinstance(mask, (list, tuple)):
            log.warning(
                _LOG_PREFIX, "mask_list_to_batch: Not a list or tuple, returning as-is"
            )
            return (mask,)
        ...
        if len(mask) == 0:
            log.warning(
                _LOG_PREFIX,
                "mask_list_to_batch: Empty list, creating default empty mask",
            )
            empty_mask = torch.zeros((1, 64, 64), dtype=torch.float32, device="cpu")
            return (empty_mask,)

        log.msg(
            _LOG_PREFIX, f"mask_list_to_batch: Processing list of {len(mask)} masks"
        )
        ...
        for i, m in enumerate(masks_3d):
            if m.shape[1:] != target_shape:
                log.msg(
                    _LOG_PREFIX,
                    f"mask_list_to_batch: Upscaling mask {i} from {m.shape[1:]} to {target_shape}",
                )
        ...
        log.msg(
            _LOG_PREFIX,
            f"mask_list_to_batch: Created batch tensor with shape {result.shape}",
        )
        return (result,)
    except Exception as e:
        log.error(
            _LOG_PREFIX, f"mask_list_to_batch conversion failed: {e}, returning as-is"
        )
        import traceback
    ...
    try:
        ...
        if isinstance(latents, dict) and "samples" in latents:
            ...
            if isinstance(samples, torch.Tensor) and samples.ndim == 4:
                log.msg(
                    _LOG_PREFIX,
                    f"latent_list_to_batch: Already a latent batch with shape {samples.shape}",
                )
                return (latents,)
        ...
        if not isinstance(latents, (list, tuple)):
            log.warning(
                _LOG_PREFIX,
                "latent_list_to_batch: Not a list or tuple, returning as-is",
            )
            return (latents,)

        if len(latents) == 0:
            log.warning(
                _LOG_PREFIX, "latent_list_to_batch: Empty list, returning as-is"
            )
            return (latents,)

        log.msg(
            _LOG_PREFIX,
            f"latent_list_to_batch: Processing list of {len(latents)} latents",
        )
        ...
        if not all(isinstance(item, dict) and "samples" in item for item in latents):
            log.warning(
                _LOG_PREFIX,
                "latent_list_to_batch: Not all items are latent dicts, returning as-is",
            )
            return (latents,)
        ...
        for i, latent2 in enumerate(latents[1:], 1):
            ...
            if samples1.shape[1:] != samples2.shape[1:]:
                log.msg(
                    _LOG_PREFIX,
                    f"latent_list_to_batch: Upscaling latent {i} from {samples2.shape[1:]} to {samples1.shape[1:]}",
                )
        ...
        log.msg(
            _LOG_PREFIX,
            f"latent_list_to_batch: Created latent batch with shape {samples1.shape}",
        )
        return (result,)
    except Exception as e:
        log.error(
            _LOG_PREFIX, f"latent_list_to_batch conversion failed: {e}, returning as-is"
        )
        import traceback
...
class legacy_ConvertToBatch(io.ComfyNode):
    @classmethod
    def define_schema(cls):
        return io.Schema(
            node_id="Convert To Batch [Eclipse]",
            display_name="⚠ Convert to Batch (Deprecated)",
            ...
            inputs=[
                ...
                io.Combo.Input(
                    ...
                    options=[
                        "IMAGE_LIST_TO_BATCH",
                        "MASK_LIST_TO_BATCH",
                        "LATENT_LIST_TO_BATCH",
                    ],
                    default="IMAGE_LIST_TO_BATCH",
                    tooltip="Target type for conversion",
                ),
            ],
            outputs=[
