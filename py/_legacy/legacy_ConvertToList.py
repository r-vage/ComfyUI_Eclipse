...
def _convert_image_batch_to_list(images):
    if not isinstance(images, (torch.Tensor, list, tuple)):
        log.warning(
            _LOG_PREFIX,
            "image_batch_to_list: Input is not a batch, returning as single-item list",
        )
        return [images]
    ...
    try:
        ...
        log.error(
            _LOG_PREFIX,
            f"image_batch_to_list conversion failed: {e}, returning as single-item list",
        )
        return [images]
...
def _convert_mask_batch_to_list(masks):
    ...
    try:
        ...
        log.error(
            _LOG_PREFIX,
            f"mask_batch_to_list conversion failed: {e}, returning as single-item list",
        )
        return [masks]
...
def _convert_latent_batch_to_list(latents):
    ...
    try:
        ...
        log.error(
            _LOG_PREFIX, f"latent_batch_to_list conversion failed: {e}, returning as-is"
        )
        return [latents]


class legacy_ConvertToList(io.ComfyNode):
    @classmethod
    def define_schema(cls):
        return io.Schema(
            node_id="Convert to List [Eclipse]",
            display_name="⚠ Convert to List (Deprecated)",
            ...
            inputs=[
                io.AnyType.Input(
                    "input", tooltip="Any value to convert to list format"
                ),
                io.Combo.Input(
                    "convert_to",
                    options=[
                        "IMAGE_BATCH_TO_LIST",
                        "MASK_BATCH_TO_LIST",
                        "LATENT_BATCH_TO_LIST",
                    ],
                    default="IMAGE_BATCH_TO_LIST",
                    tooltip="Target list conversion type",
                ),
            ],
            outputs=[
