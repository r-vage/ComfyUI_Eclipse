#
# Seed (32-bit) node - standalone seed generator with 2^32-1 range.
# Restricted range variant of Seed [Eclipse] for backends needing 32-bit seeds.
...
class RvLogic_Seed_32bit(io.ComfyNode):
    @classmethod
    def define_schema(cls):
        return io.Schema(
            node_id="Seed 32-bit [Eclipse]",
            display_name="Seed 32-bit",
            category=CATEGORY.MAIN.value + CATEGORY.PRIMITIVE.value,
            description="Standalone seed node with 32-bit range (0 to 2^32-1). Use -1 for random, -2 to increment, -3 to decrement.",
            inputs=[
                io.Int.Input("seed", default=0, min=-3, max=2**32 - 1, tooltip="Random seed (32-bit range). Use -1 for random, -2 to increment, -3 to decrement."),
            ],
            outputs=[
                io.Int.Output("SEED"),
            ],
