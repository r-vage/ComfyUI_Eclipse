...
def _read_all_prompts(file_paths: List[str]) -> List[str]:
    ...
    for file_path in file_paths:
        try:
            ...
            log.debug(
                _LOG_PREFIX,
                f"Read {len(lines)} lines from {os.path.basename(file_path)}",
            )
...
class RvText_ReadPromptFiles(io.ComfyNode):
        ...
        return io.Schema(
            ...
            inputs=[
                ...
                io.Int.Input(
                    ...
                    min=-4,
                    max=999999,
                    tooltip="Prompt index: 0+ = fixed position, -1 = random, -2 = increment, -3 = decrement, -4 = shuffle (no repeat)",
                ),
                io.Boolean.Input(
                    "stop_at_end",
                    default=True,
                    tooltip="Stop workflow when increment reaches end or decrement reaches start. Does not apply to random mode.",
                ),
                io.Boolean.Input(
                    "log_prompt",
                    default=False,
                ...
                io.Int.Input(
                    ...
                    force_input=True,
                    optional=True,
                    tooltip="When connected, special index modes (-1/-2/-3/-4) only advance when this value changes. Keep the same seed to freeze prompt selection while tweaking other workflow settings.",
                ),
            ],
            outputs=[
    ...
    def fingerprint_inputs(cls, **kwargs) -> Optional[float]:
        ...
        file_paths_numeric = (
            int(file_paths_hash, 16) * 0.000001
        )  # Convert to small float
        ...
        log.debug(
            _LOG_PREFIX,
            f"fingerprint_inputs: index={index}, max_mtime={max_mtime}, file_paths_hash={file_paths_hash}, combined={combined_hash}",
        )
    ...
    def execute(
        ...
        if index in (-1, -2, -3, -4):
            if index in (-2, -3, -4):
                log.warning(
                    _LOG_PREFIX,
                    f'Cannot {"increment" if index == -2 else "decrement" if index == -3 else "shuffle"} index from '
            ...
            log.msg(
                _LOG_PREFIX,
                f"Server-generated random seed {seed} used for random index selection.",
            )
        ...
        log.debug(
            _LOG_PREFIX, f"Processing {len(parsed_paths)} files with index {index}"
        )
        ...
        if stop_at_end:
            if original_index == -2 and index > max_index:
                ...
                log.msg(
                    _LOG_PREFIX,
                    f"Increment mode reached end of prompts ({max_index + 1} total). Stopping workflow and disabling auto-queue.",
                )
                ...
                log.msg(
                    _LOG_PREFIX,
                    f"Decrement mode reached beginning of prompts. Stopping workflow and disabling auto-queue.",
                )
                PromptServer.instance.send_sync("stop-iteration", {})
                nodes.interrupt_processing()
                return io.NodeOutput(
                ...
                log.msg(
                    _LOG_PREFIX,
                    f"Shuffle mode used all prompts ({max_index + 1} total). Stopping workflow and disabling auto-queue.",
                )
                PromptServer.instance.send_sync("stop-iteration", {})
                nodes.interrupt_processing()
                return io.NodeOutput(
        ...
        if original_index in (-1, -4):
            ...
            if max_index >= 0:
                ...
                log.debug(
                    _LOG_PREFIX,
                    f"Random seed {seed} selected index {final_index} from range 0-{max_index}",
                )
            else:
                final_index = 0
                log.debug(
                    _LOG_PREFIX,
                    f"No prompts available for random selection, using index 0",
                )
        else:
        ...
        if (
            _last_index == index
            and _last_final_index == final_index
            and _last_output is not None
            and _last_prompt_count == len(all_prompts)
        ):
            log.debug(
                _LOG_PREFIX,
                f"Using cached result for index={index}, final_index={final_index}",
            )
            return io.NodeOutput(
                _last_output,
            )
        ...
        if final_index < 0:
            ...
            log.debug(
                _LOG_PREFIX,
                f"Index {final_index} > max_index {max_index}, clamping to {max_index}",
            )
            final_index = max_index

        log.msg(
            _LOG_PREFIX,
            f"Reading line {final_index} from {len(all_prompts)} total lines (index bounds: 0-{max_index})",
        )
        ...
        log.debug(
            _LOG_PREFIX,
            f"log_prompt parameter value: {log_prompt} (type: {type(log_prompt)})",
        )
        if log_prompt:
            log.msg(
                _LOG_PREFIX,
                f"Selected line {final_index} of {max_index+1}: {selected_prompt}",
            )
        else:
            log.debug(
                _LOG_PREFIX,
                f"Selected line {final_index} of {max_index+1}: {selected_prompt[:50]}...",
            )
