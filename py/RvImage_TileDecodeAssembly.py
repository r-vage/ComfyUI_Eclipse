...
def _blend_tiles(tile1, tile2, overlap_size, direction, padding):
    ...
    if blend_size == 0:
        if direction == "horizontal":
            result = Image.new(
                "RGB", (tile1.width + tile2.width - overlap_size, tile1.height)
            )
            result.paste(
                tile1.crop((0, 0, tile1.width - overlap_size, tile1.height)), (0, 0)
            )
            result.paste(tile2, (tile1.width - overlap_size, 0))
        else:
            result = Image.new(
                "RGB", (tile1.width, tile1.height + tile2.height - overlap_size)
            )
            result.paste(
                tile1.crop((0, 0, tile1.width, tile1.height - overlap_size)), (0, 0)
            )
    ...
    if direction == "horizontal":
        ...
        crop1 = tile1.crop(
            (
                tile1.width - overlap_size + offset_left,
                0,
                tile1.width - offset_right,
                tile1.height,
            )
        )
        ...
        result.paste(
            tile1.crop((0, 0, tile1.width - overlap_size + offset_left, tile1.height)),
            (0, 0),
        )
        result.paste(blended, (tile1.width - overlap_size + offset_left, 0))
        result.paste(
            tile2.crop((offset_left + blend_size, 0, tile2.width, tile2.height)),
        ...
        crop1 = tile1.crop(
            (
                0,
                tile1.height - overlap_size + offset_top,
                tile1.width,
                tile1.height - offset_bottom,
            )
        )
        ...
        result.paste(
            tile1.crop((0, 0, tile1.width, tile1.height - overlap_size + offset_top)),
            (0, 0),
        )
        result.paste(blended, (0, tile1.height - overlap_size + offset_top))
        result.paste(
            tile2.crop((0, offset_top + blend_size, tile2.width, tile2.height)),
...
def _assemble_tiles(tiles, positions, grid_size, padding):
    ...
    for row in range(num_rows):
        ...
        for col in range(1, num_cols):
            ...
            if overlap_w > 0:
                row_img = _blend_tiles(
                    row_img, tile_img, overlap_w, "horizontal", padding
                )
            else:
                combined = Image.new(
                    "RGB",
...
class RvImage_TileDecodeAssembly(io.ComfyNode):
    ...
    def define_schema(cls):
        return io.Schema(
            ...
            inputs=[
                io.Latent.Input(
                    "samples",
                    tooltip="Latent tile samples from sampler. "
                    "Accepts both batch and list — list is auto-converted to batch.",
                ),
                io.Vae.Input("vae", tooltip="VAE for tiled decoding."),
                io.Custom(_TILE_PIPE_TYPE).Input(
                    "tile_pipe", tooltip="Tile pipe from Tile Split node."
                ),
                io.Int.Input(
                    "tile_size",
                    default=0,
            ...
            outputs=[
                io.Image.Output(
                    "image", tooltip="Reconstructed full image.", is_output_list=True
                ),
            ],
            is_input_list=True,
    ...
    def execute(cls, samples, vae, tile_pipe, tile_size, overlap, padding):
        vae = unwrap_value(vae, None)
        tile_size = unwrap_value(tile_size, 0)
        overlap = unwrap_value(overlap, 0)
        padding = unwrap_value(padding, 128)

        # Flatten tile_pipe list in case of nesting
        flat_tile_pipes = []

        def _flatten_pipes(item):
            if isinstance(item, list):
                for sub in item:
                    _flatten_pipes(sub)
            elif item is not None:
                flat_tile_pipes.append(item)

        _flatten_pipes(tile_pipe)

        # Flatten samples/latents
        latents = []

        def _flatten_samples(item):
            if isinstance(item, list):
                for sub in item:
                    _flatten_samples(sub)
            elif isinstance(item, dict) and "samples" in item:
                t = item["samples"]
                if t.is_nested:
                    t = torch.stack(t.unbind())
                for i in range(t.shape[0]):
                    latents.append(t[i : i + 1, ...])

        _flatten_samples(samples)

        if not flat_tile_pipes:
            log.error(_LOG_PREFIX, "No tile pipes received.")
            return io.NodeOutput(torch.zeros((1, 64, 64, 3)))

        assembled_images = []
        latent_idx = 0

        for tile_pipe_i in flat_tile_pipes:
            positions = tile_pipe_i.get("positions", [])
            grid_size = tile_pipe_i.get("grid_size", (1, 1))
            expected = grid_size[0] * grid_size[1]

            if latent_idx >= len(latents):
                log.warning(
                    _LOG_PREFIX,
                    f"Out of latents for tile grid: expected {expected}, but no latents remaining.",
                )
                break

            if latent_idx + expected > len(latents):
                log.warning(
                    _LOG_PREFIX,
                    f"Not enough latents for tile grid: got {len(latents) - latent_idx}, expected {expected}",
                )
                slice_latents = latents[latent_idx:]
            else:
                slice_latents = latents[latent_idx : latent_idx + expected]

            latent_idx += expected

            if not slice_latents:
                continue

            latent = torch.cat(slice_latents, dim=0)

            # Auto tile_size: use tile dimensions from pipe (decode each tile whole)
            curr_tile_size = tile_size
            if curr_tile_size <= 0:
                pipe_tile_size = tile_pipe_i.get("tile_size")
                if pipe_tile_size:
                    curr_tile_size = max(pipe_tile_size[0], pipe_tile_size[1])
                else:
                    curr_tile_size = 1024

            curr_overlap = overlap
            if curr_overlap <= 0:
                curr_overlap = max(32, curr_tile_size // 8)

            if curr_tile_size < curr_overlap * 4:
                curr_overlap = curr_tile_size // 4

            temporal_compression = vae.temporal_compression_decode()
            if temporal_compression is not None:
                temporal_size = max(2, 64 // temporal_compression)
                temporal_overlap = max(
                    1, min(temporal_size // 2, 8 // temporal_compression)
                )
            else:
                temporal_size = None
            ...
            log.msg(
                _LOG_PREFIX,
                f"Decoding grid {grid_size[0]}x{grid_size[1]} ({latent.shape[0]} tiles) — VAE tile_size={curr_tile_size}, overlap={curr_overlap}",
            )

            images = vae.decode_tiled(
                latent,
                tile_x=curr_tile_size // compression,
                tile_y=curr_tile_size // compression,
                overlap=curr_overlap // compression,
            ...
            assembled_images.append(result)

        if not assembled_images:
            return io.NodeOutput([torch.zeros((1, 64, 64, 3))])

        return io.NodeOutput(assembled_images)
