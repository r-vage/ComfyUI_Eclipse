...
class RvImage_TextImageWithFX(io.ComfyNode):
        ...
        return io.Schema(
            ...
            inputs=[
                ...
                io.String.Input(
                    ...
                    default="© Eclipse",
                    multiline=True,
                    tooltip="Text to render.",
                ),
                io.Combo.Input(
                    "font_file",
                    options=_get_font_list(),
                    default=_get_font_list()[0],
                ...
                io.Int.Input(
                    ...
                    max=2500,
                    step=1,
                    tooltip="Font size in pixels (base size before scaling).",
                ),
                io.Int.Input(
                    "text_scale",
                    default=100,
                ...
                io.String.Input(
                    "text_color", default="#ffffff", tooltip="Text fill color (hex)."
                ),
                io.Combo.Input(
                    "text_align",
                ...
                io.String.Input(
                    "stroke_color",
                    default="#000000",
                    tooltip="Text outline (stroke) color (hex).",
                ),
                # Opacity
                io.Int.Input(
                    "opacity",
    ...
    def execute(
        ...
        try:
            ...
            log.warning(
                _LOG_PREFIX, f"Could not load font '{font_file}', using default."
            )
        ...
        total_text_h = sum(m[1] for m in line_metrics) + leading * max(
            0, len(all_lines) - 1
        )
        ...
        if background_image is not None:
            ...
            if enable_shadow:
                pad = max(
                    pad,
                    abs(shadow_offset_x) + shadow_grow + shadow_blur,
        ...
        if background_image is not None:
            text_x, text_y = _compute_position(
                position,
                canvas_w,
                canvas_h,
                max_text_w,
        ...
        if enable_shadow and shadow_opacity > 0:
            ...
            if shadow_offset_x != 0 or shadow_offset_y != 0:
                shadow_mask_precomp = shift_image(
                    shadow_mask_precomp, shadow_offset_x, shadow_offset_y
                )
            if shadow_grow > 0 or shadow_blur > 0:
                sm_tensor = expand_mask(
                    image2mask(shadow_mask_precomp), shadow_grow, shadow_blur
                )
        ...
        for frame_idx in range(n_frames):
            ...
            if shadow_mask_precomp is not None:
                canvas = _composite_with_mask(
                    canvas, shadow_rgb, shadow_mask_precomp, shadow_opacity
                )
        ...
        log.msg(
            _LOG_PREFIX,
            f"Rendered text ({canvas_w}x{canvas_h}), frames={n_frames}, glow={enable_glow}, shadow={enable_shadow}",
        )
