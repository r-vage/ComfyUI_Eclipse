    ...
    def execute(cls, **kwargs):
        ...
        def add_active_path(folder_key: str, filename: Any, *, enabled: bool = True):
            ...
            if (
                ...
                and resolved not in seen_active_paths
            ):
        ...
        if verify_integrity != "off":
            ...
            if verify_integrity in {"sidecar", "verify"}:
                for path in active_paths:
                    ...
                    if expected_from_file and isinstance(
                        expected_from_file.get("sha256"), str
                    ):
        ...
        if is_standard:
            ...
            nunchaku_path = folder_paths.get_full_path(
            ...
            if not NUNCHAKU_AVAILABLE:
                ...
                try:
                    log.msg(
                        "Nunchaku Flux", f"Loading quantized model: {nunchaku_name}"
                    )
                    ...
                    log.error(
                        "Nunchaku Flux", f"Failed to load model '{nunchaku_name}': {e}"
                    )
                    loaded_model = None
                    checkpoint_name = ""
            ...
            if not NUNCHAKU_AVAILABLE:
                ...
                try:
                    ...
                    log.error(
                        "Nunchaku Qwen", f"Failed to load model '{qwen_name}': {e}"
                    )
                    loaded_model = None
                    checkpoint_name = ""
            ...
            if not NUNCHAKU_AVAILABLE:
                ...
                try:
                    ...
                    log.error(
                        "Nunchaku ZImage", f"Failed to load model '{zimage_name}': {e}"
                    )
                    loaded_model = None
                    checkpoint_name = ""
            ...
            if needs_baked_clip or needs_baked_vae or needs_vae_for_latent:
                try:
                    ...
                    log.msg(
                        _LOG_PREFIX,
                        f"Note: UNet file doesn't contain baked components: {e}",
                    )
            ...
            if use_baked_clip:
                ...
                if clip_source == "External + Model File":
                    ...
                    if is_standard and ckpt_name not in (None, "", "None"):
                        model_file_path = folder_paths.get_full_path(
            ...
            if use_baked_vae or needs_vae_for_latent:
                ...
                if vae_name in (None, "", "None"):
                    ...
                    try:
                        ...
                        log.warning(
                            _LOG_PREFIX, f"Failed to load VAE '{vae_name}': {e}"
                        )
        ...
        if configure_audio_vae:
            if audio_vae_source == "Baked":
                ...
                if is_standard and ckpt_name not in (None, "", "None"):
                    baked_model_path = folder_paths.get_full_path(
                        "checkpoints", ckpt_name
                    )
                elif is_unet and unet_name not in (None, "", "None"):
                    baked_model_path = folder_paths.get_full_path(
                ...
                if baked_model_path:
                    try:
                        ...
                        log.warning(
                            _LOG_PREFIX, f"Failed to extract baked audio VAE: {e}"
                        )
                else:
                    log.warning(
                        _LOG_PREFIX,
                ...
                if audio_vae_name in (None, "", "None"):
                    ...
                    try:
                        audio_vae_path = folder_paths.get_full_path(
                            "vae", audio_vae_name
                        )
                        loaded_audio_vae = (
                            load_audio_vae_from_path(audio_vae_path)
                            if audio_vae_path
                            else None
                        )
                        if loaded_audio_vae is None:
                        ...
                        log.warning(
                            _LOG_PREFIX,
                            f"Failed to load audio VAE '{audio_vae_name}': {e}",
                        )
