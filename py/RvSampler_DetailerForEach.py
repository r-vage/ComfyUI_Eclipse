...
class RvSampler_DetailerForEach(io.ComfyNode):
    ...
    def define_schema(cls):
        return io.Schema(
            ...
            inputs=[
                ...
                io.Boolean.Input("inpaint_model", default=True, label_on="enabled", label_off="disabled", tooltip="Force using inpaint model."),
                io.Int.Input("noise_mask_feather", default=20, min=0, max=100, tooltip="Feather level for noise mask."),
                io.Boolean.Input("tiled_encode", default=True, label_on="enabled", label_off="disabled", tooltip="Use tiled encode."),
                io.Boolean.Input("tiled_decode", default=True, label_on="enabled", label_off="disabled", tooltip="Use tiled decode."),
                io.Int.Input("seed", default=0, min=-3, max=0xffffffffffffffff, tooltip="Seed for sampler. Use -1 for random, -2 to increment, -3 to decrement."),
    ...
    def execute(cls, image, segs, guide_size, guide_size_for, max_size, seed, steps, cfg, sampler_name, scheduler,
                denoise, feather, noise_mask, force_inpaint, basic_pipe, cycle,
        ...
        for idx, img_i in enumerate(image_list):
            ...
            enhanced_img, cropped, cropped_enhanced, cropped_enhanced_alpha, cnet_pil_list, new_segs = \
                DetailerForEach.do_detail(img_i, filtered_segs, model, clip, vae, guide_size, guide_size_for, max_size, seed, steps, cfg,
                                          ...
                                          refiner_ratio=0.0, refiner_model=None,
                                          refiner_clip=None, refiner_positive=None,
        ...
        return io.NodeOutput(enhanced_images, [(segs_val[0], detailed_segs_list)], [basic_pipe_val], ui={"seed": [seed]})
