...
class RvMask_Preview(io.ComfyNode):
    @classmethod
    ...
    def execute(cls, masks):
        ...
        if hasattr(first_mask, "shape"):
            ...
            while temp_mask.ndim > 2:
                temp_mask = (
                    temp_mask.squeeze(0)
                    if temp_mask.shape[0] == 1
                    else temp_mask.squeeze()
                )
            if temp_mask.ndim >= 2:
        ...
        full_output_folder, filename, counter, subfolder, prefix = (
            folder_paths.get_save_image_path(prefix, _output_dir, width, height)
        )
        results = []
        ...
        for batch_number, mask in enumerate(masks):
            ...
            img.save(
                os.path.join(full_output_folder, file), compress_level=_compress_level
            )
