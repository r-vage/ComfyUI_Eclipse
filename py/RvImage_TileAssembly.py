...
class RvImage_TileAssembly(io.ComfyNode):
    ...
    def execute(cls, tiles, tile_pipe, padding):
        padding = unwrap_value(padding, 64)
        ...
        # Flatten tiles (can be lists of tensors or batches of tensors)
        flat_tiles = [t.squeeze(0) for t in flatten_images(tiles)]
        ...
        for tile_pipe_i in flat_tile_pipes:
            ...
            expected = num_cols * num_rows

            if tile_idx >= len(flat_tiles):
                break

            if tile_idx + expected > len(flat_tiles):
                slice_tiles = flat_tiles[tile_idx:]
            else:
                slice_tiles = flat_tiles[tile_idx : tile_idx + expected]
            ...
            for row in range(num_rows):
                if row * num_cols >= len(slice_tiles):
                    break
                row_img = _tensor2pil(slice_tiles[row * num_cols])
                for col in range(1, num_cols):
                    idx = row * num_cols + col
                    if idx >= len(slice_tiles):
                        break
                    tile_img = _tensor2pil(slice_tiles[idx])
            ...
            for row in range(1, num_rows):
                ...
                if overlap_h > 0:
                    final = _blend_tiles(
                        final, row_images[row], overlap_h, "vertical", padding
                    )
                else:
                    combined = Image.new(
                        "RGB",
