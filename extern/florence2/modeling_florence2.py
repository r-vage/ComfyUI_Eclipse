...
import torch.utils.checkpoint #type: ignore
...
# ---------------------------------------------------------------------------
# Attention mask helpers — local implementations so we don't depend on the
# deprecated `transformers.modeling_attn_mask_utils.AttentionMaskConverter`
# API (removed in Transformers ≥ v5.10).  The logic below is extracted
# verbatim from the transformers 4.x / 5.x source and is fully compatible
# with both versions.
# ---------------------------------------------------------------------------

def _expand_mask(mask: torch.Tensor, dtype: torch.dtype, tgt_len: int | None = None) -> torch.Tensor:
    """Expand [bsz, src_len] attention mask to [bsz, 1, tgt_len, src_len]."""
...
def _make_causal_mask(
    input_ids_shape: torch.Size | tuple,
    dtype: torch.dtype,
    device: torch.device,
    past_key_values_length: int = 0,
    sliding_window: int | None = None,
) -> torch.Tensor:
    """Lower-triangular causal mask of shape [bsz, 1, tgt_len, tgt_len + past]."""
    ...
    if sliding_window is not None:
        ...
        ctx_mask = torch.tril(torch.ones_like(mask, dtype=torch.bool), diagonal=diagonal)
        mask.masked_fill_(ctx_mask, torch.finfo(dtype).min)
    return mask[None, None, :, :].expand(bsz, 1, tgt_len, tgt_len + past_key_values_length)


def _prepare_4d_attention_mask(mask: torch.Tensor, dtype: torch.dtype, tgt_len: int | None = None) -> torch.Tensor:
    """Expand [bsz, seq_len] → [bsz, 1, tgt_seq_len, src_seq_len] (non-causal)."""
    return _expand_mask(mask, dtype, tgt_len=tgt_len)


def _prepare_4d_attention_mask_for_sdpa(
    mask: torch.Tensor, dtype: torch.dtype, tgt_len: int | None = None
) -> torch.Tensor | None:
    """Same as _prepare_4d_attention_mask but returns None when the mask is all-ones (SDPA optimisation)."""
    _, key_value_length = mask.shape
    tgt_len = tgt_len if tgt_len is not None else key_value_length
    is_tracing_ = torch.jit.is_tracing()
    if not is_tracing_ and torch.all(mask == 1):  # type: ignore[arg-type]
        return None
    return _expand_mask(mask, dtype, tgt_len=tgt_len)
...
def _prepare_4d_causal_attention_mask(
    ...
    """Build a causal 4-D mask [bsz, 1, query_len, key_value_len]."""
    dtype = inputs_embeds.dtype
    device = inputs_embeds.device
    query_len = input_shape[-1]
    key_value_length = query_len + past_key_values_length

    if attention_mask is not None and attention_mask.dim() == 4:
        expected = (input_shape[0], 1, query_len, key_value_length)
        if tuple(attention_mask.shape) != expected:
            raise ValueError(
                f"Incorrect 4D attention_mask shape: {tuple(attention_mask.shape)}; expected: {expected}."
            )
        inverted = 1.0 - attention_mask
        return inverted.masked_fill(inverted.to(torch.bool), torch.finfo(dtype).min)

    causal = None
    if query_len > 1 or sliding_window is not None:
        causal = _make_causal_mask(
            (input_shape[0], query_len), dtype, device=device,
            past_key_values_length=past_key_values_length, sliding_window=sliding_window,
        )

    if attention_mask is not None and attention_mask.dim() == 2:
        expanded = _expand_mask(attention_mask, dtype, tgt_len=query_len).to(device)
        if causal is not None:
            causal = causal.masked_fill(expanded.bool(), torch.finfo(dtype).min)
        else:
            causal = expanded
    elif causal is None:
        # query_len == 1 and no attention_mask: nothing to mask
        return None
...
def _prepare_4d_causal_attention_mask_for_sdpa(
    ...
    """SDPA variant: returns None where possible to allow the flash-attention fast path."""
    query_len = input_shape[-1]
    key_value_length = query_len + past_key_values_length
    is_tracing_ = torch.jit.is_tracing()

    # Determine if the causal mask can be skipped entirely
    can_skip = False
    if attention_mask is None:
        if (query_len == 1 or key_value_length == query_len) and (
            sliding_window is None or key_value_length < sliding_window
        ):
            can_skip = not is_tracing_
    else:
        if attention_mask.dim() != 4 and not is_tracing_ and torch.all(attention_mask == 1):
            if (query_len == 1 or key_value_length == query_len) and (
                sliding_window is None or key_value_length < sliding_window
            ):
                can_skip = True
    ...
    # Unmask fully-masked rows so SDPA memory-efficient path doesn't under-attend
    if mask is not None and not is_tracing_ and inputs_embeds.device.type in ("cuda", "xpu"):
        min_val = torch.finfo(inputs_embeds.dtype).min
        mask = mask.mul(~torch.all(mask == min_val, dim=-1, keepdim=True))
    return mask
