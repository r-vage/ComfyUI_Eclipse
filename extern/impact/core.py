        ...
        if use_acn:
            ...
            if self.vae is not None:
                ...
                if 'vae' in signature.parameters:
                    ...
                    logging.error("[Eclipse Impact] ERROR: The ComfyUI version is outdated. VAE cannot be used in ApplyControlNet.")
                    raise Exception("[Eclipse Impact] ERROR: The ComfyUI version is outdated. VAE cannot be used in ApplyControlNet.")
            else:
...
def enhance_detail(image, model, clip, vae, guide_size, guide_size_for_bbox, max_size, bbox, seed, steps, cfg,
    ...
    if wildcard_opt is not None and wildcard_opt != "":
        logging.warning("[Eclipse Impact] Wildcard features are ignored in this stripped version.")
    ...
    if detailer_hook is None or not detailer_hook.get_skip_sampling():
        ...
        if vae_tiled_decode:
            ...
            logging.info(f"[Eclipse Impact] vae decoded (tiled) in {time.time() - start:.1f}s")
        else:
            try:
                refined_image = vae.decode(refined_latent['samples'])
            except Exception:
                # usually an out-of-memory exception from the decode, so try a tiled approach
                logging.warning(f"[Eclipse Impact] failed after {time.time() - start:.1f}s, doing vae.decode_tiled 64...")
