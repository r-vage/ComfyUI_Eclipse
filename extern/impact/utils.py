...
def dilate_mask(mask, dilation_factor, iter=1):
    ...
    if use_gpu_opencv():
        mask = cv2.UMat(mask)  # type: ignore
        kernel = cv2.UMat(kernel)  # type: ignore

    if dilation_factor > 0:
        result = cv2.dilate(mask, kernel, iterations=iter)  # type: ignore
    else:
        result = cv2.erode(mask, kernel, iterations=iter)  # type: ignore
...
def dilate_masks(segmasks, dilation_factor, iter=1):
    ...
    for i in range(len(segmasks)):
        ...
        if use_gpu_opencv():
            cv2_mask = cv2.UMat(cv2_mask)  # type: ignore

        if dilation_factor > 0:
            dilated_mask = cv2.dilate(cv2_mask, kernel, iterations=iter)  # type: ignore
        else:
            dilated_mask = cv2.erode(cv2_mask, kernel, iterations=iter)  # type: ignore
