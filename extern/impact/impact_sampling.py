...
from comfy import samplers  # type: ignore
import comfy.samplers  # type: ignore
import comfy.sample  # type: ignore
import comfy.utils  # type: ignore
import comfy.model_management  # type: ignore
from comfy_extras import nodes_custom_sampler  # type: ignore
...
try:
    from comfy_extras.nodes_custom_sampler import Noise_EmptyNoise, Noise_RandomNoise  # type: ignore
...
def calculate_sigmas(model, sampler, scheduler, steps):
    ...
    if scheduler.startswith("AYS"):
        ...
        sigmas = nodes.NODE_CLASS_MAPPINGS["GITSScheduler"]().execute(
            float(scheduler[11:-1]), steps, denoise=1.0
        )[0]
    elif scheduler == "LTXV[default]":
        sigmas = nodes.NODE_CLASS_MAPPINGS["LTXVScheduler"]().execute(
            20, 2.05, 0.95, True, 0.1
        )[0]
    elif scheduler.startswith("OSS"):
        sigmas = nodes.NODE_CLASS_MAPPINGS["OptimalStepsScheduler"]().execute(
            scheduler[4:], steps, denoise=1.0
        )[0]
    else:
        ...
        def sampler_function_wrapper(model, x, sigmas, **kwargs):
            if "noise_sampler" not in kwargs:
                kwargs["noise_sampler"] = get_noise_sampler(
                    x, "gpu" not in sampler_name, total_sigmas, **kwargs
                )
...
class KSamplerAdvancedWrapper:
    ...
    def clone_with_conditionings(self, positive, negative):
        ...
        return KSamplerAdvancedWrapper(
            model, cfg, sampler_name, scheduler, positive, negative, self.sampler_opt
        )
    ...
    def sample_advanced(
        ...
        return_with_leftover_noise,
        hook=None,
        recovery_mode="ratio additional",
        recovery_sampler="AUTO",
        recovery_sigma_ratio=1.0,
        noise=None,
    ):

        model, cfg, sampler_name, scheduler, positive, negative, sigma_factor = (
            self.params
        )
        ...
        if recovery_mode != "DISABLE" and sampler_name in [
            "uni_pc",
            "uni_pc_bh2",
            "dpmpp_sde",
            "dpmpp_sde_gpu",
        ...
        if (
            recovery_sigma_ratio > 0
            and recovery_mode != "DISABLE"
            and sampler_name
            in [
                "uni_pc",
                "uni_pc_bh2",
            ...
            if recovery_sampler == "AUTO":
                recovery_sampler = (
                    "dpm_fast"
                    if sampler_name
                    in ["uni_pc", "uni_pc_bh2", "dpmpp_sde", "dpmpp_sde_gpu"]
                    else "dpmpp_2m"
                )
            ...
            latent_image = latent_compositor.composite(
                base_image, latent_image, 0, 0, False, noise_mask
            )[0]
            ...
            try:
                latent_image = separated_sample(
                    ...
                    negative,
                    latent_image,
                    start_at_step - compensate,
                    end_at_step,
                    return_with_leftover_noise,
                    sigma_ratio=recovery_sigma_ratio * sigma_factor,
                    sampler_opt=self.sampler_opt,
                    scheduler_func=self.scheduler_func,
                )
            except ValueError as e:
