...
def read_config():
    try:
        ...
        if not os.path.exists(custom_wildcards_path):
            logging.warning(f"[Eclipse Impact] custom_wildcards path not found: {custom_wildcards_path}. Using default path.")
        ...
        if 'wildcard_cache_limit_mb' in default_conf:
            try:
                ...
                logging.warning(f"[Eclipse Impact] Invalid wildcard_cache_limit_mb value: {default_conf['wildcard_cache_limit_mb']}. Using default: 50")
                cache_limit_mb = 50

        return {
                    'sam_editor_cpu': default_conf['sam_editor_cpu'].lower() == 'true' if 'sam_editor_cpu' in default_conf else False,
                    'sam_editor_model': default_conf['sam_editor_model'].lower() if 'sam_editor_model' in default_conf else 'sam_vit_b_01ec64.pth',
