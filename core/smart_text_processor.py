...
class SmartTextProcessor:
    ...
    def __init__(self, patterns_dir: Optional[str] = None):
        ...
        self.protected_patterns: Dict[str, re.Pattern] = (
            {}
        )  # Protected patterns (detect but don't remove)
        self.sentence_patterns: Dict[str, List[re.Pattern]] = (
            {}
        )  # Sentence patterns per category
        ...
        log.debug(
            _LOG_PREFIX, f"Initialized with {len(self.compiled)} compiled patterns"
        )
    ...
    def _load_index(self) -> None:
        ...
        log.debug(
            _LOG_PREFIX, f"Loaded index with {len(self.index.get('files', {}))} files"
        )
    ...
    def _load_cleanup_patterns(self) -> None:
        ...
        if os.path.exists(cleanup_path):
            ...
            log.debug(
                _LOG_PREFIX,
                f"Loaded cleanup patterns: {list(self.cleanup_rules.keys())}",
            )
        else:
            log.warning(
                _LOG_PREFIX, f"Cleanup file not found: {cleanup_path}, using defaults"
            )
            self.cleanup_rules = {}
    ...
    def _parse_template(self, template: str, data: Dict) -> str:
        ...
        result = re.sub(
            r"\)\? ", r")?", result
        )  # Remove space immediately after optional group
    ...
    def _compile_category(self, category: str, data: Dict) -> None:
        ...
        if sentence_pats:
            ...
            log.debug(
                _LOG_PREFIX,
                f"Raw sentence_pats for {category}: {len(sentence_pats)} patterns",
            )
            if sentence_pats:
                log.debug(
                    _LOG_PREFIX, f"First raw pattern: {sentence_pats[0][:100]}..."
                )
            for pat in sentence_pats:
                try:
                    ...
                    log.warning(
                        _LOG_PREFIX, f"Invalid sentence pattern in {category}: {e}"
                    )
            log.debug(
                _LOG_PREFIX,
                f"Compiled {category} with {len(self.sentence_patterns[category])} sentence patterns",
            )
    ...
    def _compile_from_presets(
        self, category: str, data: Dict, presets: List[Dict]
    ) -> None:
        ...
        for preset in sorted_presets:
            ...
            if preset.get("protected", False):
                ...
                log.debug(
                    _LOG_PREFIX,
                    f"  Protected preset '{preset.get('name', 'unnamed')}': {pattern[:80]}...",
                )
            else:
                regular_patterns.append(pattern)
                log.debug(
                    _LOG_PREFIX,
                    f"  Preset '{preset.get('name', 'unnamed')}': {pattern[:80]}...",
                )
        ...
        if regular_patterns:
            ...
            try:
                self.compiled[category] = re.compile(
                    rf"(?P<term>{combined})", re.IGNORECASE
                )
                log.debug(
                    _LOG_PREFIX,
                    f"Compiled {category} with {len(regular_patterns)} preset patterns",
                )
            except re.error as e:
        ...
        if protected_patterns:
            ...
            try:
                self.protected_patterns[category] = re.compile(
                    rf"(?P<term>{combined})", re.IGNORECASE
                )
                log.debug(
                    _LOG_PREFIX,
                    f"Compiled {category} with {len(protected_patterns)} PROTECTED patterns",
                )
            except re.error as e:
    ...
    def _compile_from_terms(self, category: str, data: Dict) -> None:
        ...
        if terms:
            ...
            try:
                self.compiled[category] = re.compile(
                    rf"(?P<term>(?<!-)\b(?:{alternation})\b(?!-))", re.IGNORECASE
                )
                log.debug(
                    _LOG_PREFIX, f"Compiled {category} with {len(terms)} flat terms"
                )
            except re.error as e:
                log.error(_LOG_PREFIX, f"Failed to compile {category}: {e}")
    ...
    def detect(
        ...
        log.debug(
            _LOG_PREFIX,
            f"detect() found {len(matches)} matches in {len(cats)} categories",
        )
        if matches:
            for m in matches:
                log.debug(
                    _LOG_PREFIX,
                    f"  match: '{m['text']}' cat={m['category']} span={m['span']} protected={m.get('protected', False)}",
                )
        return matches
    ...
    def detect_sentences(
        ...
        log.debug(
            _LOG_PREFIX,
            f"detect_sentences() available sentence_patterns keys: {list(self.sentence_patterns.keys())}",
        )
        ...
        for cat in cats:
            if cat not in self.sentence_patterns:
                log.debug(
                    _LOG_PREFIX,
                    f"detect_sentences() category '{cat}' NOT in sentence_patterns, skipping",
                )
                continue
            ...
            log.debug(
                _LOG_PREFIX,
                f"detect_sentences() checking {pattern_count} patterns for category '{cat}'",
            )
            ...
            if lighting_pos >= 0:
                ...
                log.debug(
                    _LOG_PREFIX,
                    f"detect_sentences() Found 'The lighting' at pos {lighting_pos}",
                )
                log.debug(
                    _LOG_PREFIX, f"detect_sentences() Context: ...{repr(context)}..."
                )
                ...
                if lighting_pos > 0:
                    ...
                    log.debug(
                        _LOG_PREFIX,
                        f"detect_sentences() Char before 'The lighting': {repr(char_before)} (ord={ord(char_before)})",
                    )

                # Test simple pattern without lookbehind
                simple_pat = re_debug.compile(
                    r"The (?:light|lighting|illumination) (?:is|appears)",
                    re_debug.IGNORECASE,
                )
                simple_match = simple_pat.search(text)
                log.debug(
                    _LOG_PREFIX,
                    f"detect_sentences() Simple pattern test: {'MATCH' if simple_match else 'NO MATCH'}",
                )
            else:
                log.debug(
                    _LOG_PREFIX, f"detect_sentences() 'The lighting' NOT found in text!"
                )
            ...
            if self.sentence_patterns[cat]:
                ...
                log.debug(
                    _LOG_PREFIX,
                    f"detect_sentences() First pattern for {cat}: {first_pat.pattern[:100]}...",
                )
                test_match = first_pat.search(text)
                log.debug(
                    _LOG_PREFIX,
                    f"detect_sentences() First pattern direct test: {'MATCH' if test_match else 'NO MATCH'}",
                )
                if test_match:
                    log.debug(
                        _LOG_PREFIX,
                        f"detect_sentences() Match text: {test_match.group()[:60]}...",
                    )
            ...
            for i, pattern in enumerate(self.sentence_patterns[cat]):
                ...
                if found:
                    log.debug(
                        _LOG_PREFIX,
                        f"detect_sentences() pattern {i} matched {len(found)} times",
                    )
                for m in found:
                    matches.append(
                        {
        ...
        log.debug(
            _LOG_PREFIX, f"detect_sentences() found {len(matches)} sentence matches"
        )
        return matches
    ...
    def remove_matches(
        ...
        for start, end in sorted(removal_spans, reverse=True):
            ...
            needs_space = (
                start > 0
                and end < len(s)  # Not at boundaries
                and s[start - 1 : start]
                not in (" ", "\t", "\n", "")  # Char before span
                and s[end : end + 1] not in (" ", "\t", "\n", "")  # Char after span
            )
    ...
    def soften_matches(
        ...
        text: str,
        matches: List[Dict[str, Any]],
        soften_map: Optional[Dict[str, str]] = None,
    ) -> str:
