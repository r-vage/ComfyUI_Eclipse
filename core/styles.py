...
def _load_styles_cached(
    ...
    if (
        ...
        and _styles_cache["styles_by_mode"]
        and (now - _styles_cache["last_check"]) < _STYLES_CACHE_TTL
    ):
    ...
    if (
        current_mtime == _styles_cache["last_mtime"]
        and _styles_cache["styles_by_mode"]
        and _styles_cache["directory"] == directory
    ):
        return _styles_cache["styles_by_mode"], _styles_cache["names_by_mode"]
...
def read_csv_file(file_path: str) -> Optional[List[Dict[str, Any]]]:
    ...
    try:
        ...
        log.error(
            _LOG_PREFIX, f"An error occurred while reading CSV {file_path}: {str(e)}"
        )
        return None
...
def detect_style_type(prompt: str) -> Optional[str]:
    ...
    if "{prompt}" in prompt:
        suffix = (
            prompt.split("{prompt}", 1)[1].strip()
            if len(prompt.split("{prompt}", 1)) > 1
            else ""
        )
    else:
        suffix = ""
...
def load_styles_from_directory(
    directory: str,
) -> Tuple[Dict[str, List[Dict[str, Any]]], Dict[str, List[str]]]:
    ...
    styles_by_mode: Dict[str, List[Dict[str, Any]]] = {
        "tag_based": [],
        "natural_language": [],
        "custom": [],
    }
    names_by_mode: Dict[str, List[str]] = {
        "tag_based": [],
        "natural_language": [],
        "custom": [],
    }
    ...
    custom_styles: List[Dict[str, Any]] = (
        []
    )  # Collect custom styles for processing after main files
    ...
    for style_file in all_files:
        ...
        for item in style_data:
            ...
            if (
                style_name.strip().lower() == "base"
                and (prompt or "").strip() == "{prompt}"
            ):
...
def find_template_by_name(
    json_data: List[Dict[str, Any]], template_name: str
