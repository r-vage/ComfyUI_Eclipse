...
class FileListCache:
    ...
    @classmethod
    def get_cache_key(
        cls, folder_path: str, include_subfolders: bool, sort_by: str, sort_order: str
    ) -> str:
    ...
    @classmethod
    def set_cached_list(
        cls, cache_key: str, file_list: List[str], params: Dict[str, Any]
    ) -> None:
        # Cache a file list.
        cls._cache[cache_key] = file_list.copy()  # Store a copy to prevent modification
        cls._cache_params[cache_key] = params.copy()

    _on_invalidate_callbacks = set()

    @classmethod
    def register_invalidate_callback(cls, cb) -> None:
    ...
    def invalidate(cls, folder_path: Optional[str] = None) -> None:
        ...
        if folder_path is None:
            ...
            keys_to_remove = [
                k for k in cls._cache.keys() if k.startswith(folder_path + "|")
            ]
        ...
        # Trigger callbacks
        for cb in list(cls._on_invalidate_callbacks):
            try:
                cb(folder_path)
            except Exception:
                pass
