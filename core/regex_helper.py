...
def _get_nsfw_patterns() -> Dict[str, Any]:
    ...
    try:
        ...
        _nsfw_patterns_cache = {"x_rated": x_rated_terms, "mature": mature_terms}
        _nsfw_json_mtime = current_mtime

        log.debug(
            _LOG_PREFIX,
            f"Loaded NSFW patterns from {json_path}: {len(x_rated_terms)} X-rated, {len(mature_terms)} Mature terms",
        )
        return _nsfw_patterns_cache
...
RE_AGE_RANGE_OR = re.compile(
    r"\b(late\s+teens)\s+or\s+(early\s+twenties)\b", re.IGNORECASE
)
RE_AGE_RANGE_NUMERIC = re.compile(
    r"\bin\s+(?:her|his|their)\s+(early|mid|late)\s+(\d{2})s\b", re.IGNORECASE
)
...
RE_UNDERAGE_GIRL = re.compile(
    r"\b(?:young|cute|adorable)\s+(?:girl|girls)\b|\b(?:girl|girls)\b", re.IGNORECASE
)
RE_UNDERAGE_BOY = re.compile(
    r"\b(?:young|cute|adorable)\s+(?:boy|boys)\b|\b(?:boy|boys)\b", re.IGNORECASE
)
...
RE_UNDERAGE_TEEN = re.compile(r"\b(?:teen|teenager|teenagers|teenage)\b", re.IGNORECASE)
...
def is_tags_format(text: str) -> bool:
    ...
    has_verbs = bool(
        re.search(r"\b(is|are|was|were|has|have|shows|depicts)\s+", text, re.IGNORECASE)
    )
...
def smart_phrase_removal(text: str, patterns: list, removal_type: str) -> str:
    ...
    if text != original:
        log.debug(
            _LOG_PREFIX,
            f"Smart removal ({removal_type}): {len(original) - len(text)} chars removed",
        )
...
def replace_underage_terms(text: str, target_age: int) -> str:
    ...
    text = re.sub(
        ...
        lambda m: get_age_descriptor(target_age, "female")
        + ("s" if m.group(1).endswith("s") else ""),
        text,
        flags=re.IGNORECASE,
    )
    text = re.sub(
        r"\bteenage\s+(boy|boys)\b",
        lambda m: get_age_descriptor(target_age, "male")
        + ("s" if m.group(1).endswith("s") else ""),
        text,
        flags=re.IGNORECASE,
    )
    text = re.sub(
        ...
        lambda m: get_age_descriptor(target_age, "neutral")
        + ("s" if m.group(1) in ("children", "kids") else ""),
        text,
        flags=re.IGNORECASE,
    )
    ...
    for match in reversed(list(RE_UNDERAGE_GIRL.finditer(text))):
        ...
        if (
            match.start() > 0
            and text[match.start() - 1 : match.start()] in "abcdefghijklmnopqrstuvwxyz"
        ):
            continue
        if (
            match.end() < len(text)
            and text[match.end() : match.end() + 1] in "abcdefghijklmnopqrstuvwxyz"
        ):
            continue
    ...
    for match in reversed(list(RE_UNDERAGE_TEEN.finditer(text))):
        ...
        if matched_text in ("teen", "teenager", "teenagers"):
            descriptor = (
                f"{target_age}-year-old {get_age_descriptor(target_age, gender)}"
            )
...
def adjust_age(text: str, target_age: int) -> str:
    ...
    if not (18 <= target_age <= 99):
        log.warning(
            _LOG_PREFIX,
            f"Invalid target age {target_age}, must be 18-99. Using default 25.",
        )
        target_age = 25
    ...
    text = RE_AGE_APPEARS_DESCRIPTOR.sub(
        f"who appears to be {target_age} years old", text
    )
    ...
    if text != original_text:
        ...
        log.debug(
            _LOG_PREFIX, f"Adjusted age to {target_age}, {changes} characters changed"
        )
...
def validate_age_adjustment(text: str) -> Tuple[bool, str]:
    ...
    for pattern in [
        RE_NUMERIC_AGE_HYPHENATED,
        RE_NUMERIC_AGE_SPACED,
        RE_NUMERIC_AGE_YO,
    ]:
