...
def get_hf_file_hash(repo_id: str, filename: str, token: Optional[str] = None) -> str | None:
...
def check_model_completeness(model_path: Path, repo_id: Optional[str] = None, hf_token: Optional[str] = None) -> Tuple[bool, List[str]]:
    ...
    if not index_file:
        ...
        if repo_id:
            try:
                ...
                if found_remote_index:
                    ...
                    weight_suffix = ".safetensors" if "safetensors" in found_remote_index else ".bin"
                    for f in repo_files:
                        if f.endswith(weight_suffix) and "consolidated" not in f.lower():
                            if not (model_path / f).exists():
                                missing_files.append(f)
                else:
                    # No index file exists in the repo either. Just check all files in repo_files
                    for f in repo_files:
                        if f.startswith(".") or f.lower() in ("readme.md", "licence", "license"):
                            continue
                        if "consolidated" in f.lower():
                            continue
                        is_essential = f.endswith(".json") or f.endswith(".txt") or f.endswith(".model")
                        is_weight = f.endswith(".safetensors") or f.endswith(".bin") or f.endswith(".gguf") or f.endswith(".pt") or f.endswith(".pth") or f.endswith(".ckpt") or f.endswith(".onnx")
                        if is_essential or is_weight:
                            if not (model_path / f).exists():
                                missing_files.append(f)
            except Exception as e:
                log.warning(_LOG_PREFIX, f"Could not list repo files for completeness check: {e}")
        ...
        if not missing_files:
            ...
            weight_extensions = (".safetensors", ".bin", ".gguf", ".pt", ".pth", ".ckpt")
            has_weights = any(
                f.name.endswith(weight_extensions) for f in model_path.iterdir() if f.is_file()
            )
            if not has_weights:
                log.warning(_LOG_PREFIX, f"Model folder {model_path} has config.json but no weights found.")
                return (False, ["model.safetensors"])  # request at least one weights file
            
            return (True, [])
        else:
            log.warning(_LOG_PREFIX, f"Model incomplete (repo scan): {len(missing_files)} file(s) missing")
    ...
    def _download_model(target_path: Path) -> bool:
        ...
        if is_direct_url:
            ...
            if 'huggingface.co' in repo_id and len(parts) >= 6:
                ...
                expected_hash = get_hf_file_hash(hf_repo_id, filename, hf_token) if hf_repo_id else None
        ...
        if not target.exists():
            try:
                ...
                if target.exists():
                    ...
                    is_complete, missing = check_model_completeness(target, extract_repo_id_from_url(repo_id) if repo_id else None, hf_token)
                    if not is_complete:
