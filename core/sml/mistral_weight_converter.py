...
def load_hf_weights(model_path: Path) -> Dict[str, "torch.Tensor"]:
    ...
    if shard_files:
        log.msg(
            _LOG_PREFIX, f"  Loading weights from {len(shard_files)} sharded files..."
        )
...
def convert_weights_to_mistral(
    model_path: str, output_path: Optional[str] = None, dry_run: bool = False
) -> Tuple[bool, str]:
    ...
    if not is_hf_format_model(model_dir):
        ...
        if (model_dir / "consolidated.safetensors").exists():
            return (
                True,
                "Model already has consolidated.safetensors (Mistral-native format)",
            )
    ...
    try:
        ...
        if unmapped_keys:
            log.msg(
                _LOG_PREFIX,
                f"  Warning: {len(unmapped_keys)} keys could not be mapped:",
            )
        ...
        if is_fp8:
            ...
            log.error(
                _LOG_PREFIX,
                "    FP8 HuggingFace models cannot be converted to Mistral-native format",
            )
            log.error(
                _LOG_PREFIX,
                "    because they're missing attention calibration tensors (fake_quantizer)",
            )
            log.error(_LOG_PREFIX, "    that are required by vLLM's Mistral loader.")
            log.error(_LOG_PREFIX, "")
            log.error(_LOG_PREFIX, "  Options:")
            log.error(
                _LOG_PREFIX, "    1. Use the BF16 version of this model (if available)"
            )
            log.error(
                _LOG_PREFIX,
                "    2. Use the original Mistral-native FP8 model (non-abliterated)",
            )
            log.error(
                _LOG_PREFIX, "    3. Use 'Transformers' backend with transformers>=5.0"
            )
            return (
                False,
                "FP8 HuggingFace models cannot be converted (missing attention calibration data)",
            )

        if dry_run:
            return (
                True,
                f"Dry run: Would convert {mapped_count} keys ({len(unmapped_keys)} unmapped)",
            )

        # Save as consolidated.safetensors
        output_file = (
            Path(output_path) if output_path else model_dir / "consolidated.safetensors"
        )
        log.msg(_LOG_PREFIX, f"  Saving to {output_file.name}...")

        # Calculate size
        total_bytes = sum(
            t.numel() * t.element_size() for t in mistral_weights.values()
        )
        ...
        return (
            True,
            f"Successfully converted {mapped_count} weight tensors to {output_file.name}",
        )
