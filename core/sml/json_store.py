# Durable JSON-object persistence for shared Smart LM configuration files.
...
def _interprocess_lock(path: Path) -> Iterator[None]:
    # Keep one persistent lock inode so replacing the JSON file cannot split locks.
...
def locked_path(path: str | os.PathLike[str]) -> Iterator[None]:
    # Hold the shared thread/process lock across a caller-owned transaction.
